...
parse_not_supported_version_test() ->
  ?assertEqual({error, unsupported_version},
               iot_protocol:parse(<<0, 0, 0, 11, 20, 5, 0, 0, 0, 0, 0>>)).

parse_not_payload_test() ->
  assert_message(<<0, 0, 0, 11, 1, 5, 0, 0, 0, 0, 0>>, iot_protocol_obj:new(1, 5)),
  assert_message(<<0, 0, 0, 11, 1, 3, 0, 0, 0, 0, 0>>, iot_protocol_obj:new(3)).

parse_with_payload_test() ->
  assert_message(<<0, 0, 0, 16, 1, 3, 0, 0, 0, 0, 0, 1, 2, 3, 4, 5>>,
                 iot_protocol_obj:new(1, 3, <<1, 2, 3, 4, 5>>)).
...
assemble_unsupported_version_test() ->
  ?assertEqual({error, unsupported_version},
               iot_protocol:assemble(iot_protocol_obj:new(12, 210))).
...
assert_assemble(Binary, Message) ->
  ?assertEqual({ok, Binary}, iot_protocol:assemble(Message)).

assert_message(Message, ParsedMessage) ->
  ?assertEqual({ok, ParsedMessage}, iot_protocol:parse(Message)).